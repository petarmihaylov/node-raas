...
export default class Pull extends Command {
  ...
  async run(): Promise<void> {
    ...
    if (
      flags.baseEndpointUrl === undefined ||
      ...
      responses = await inquirer.prompt([
        ...
        },
        {
          ...
          type: 'password',
          when: (responses: any) =>
            responses.maskPassword === true && flags.password === undefined,
          mask: '*',
          validate: (input) =>
            input.length > 0
              ? true
        ...
        },
        {
          ...
          type: 'input',
          when: (responses: any) =>
            responses.maskPassword === false && flags.password === undefined,
          validate: (input) =>
            input.length > 0
              ? true
              : 'Password cannot be blank. Please enter a password.',
        ...
        },
        {
          name: 'verbose',
          message:
            'WARNING: Earlier you selected to hide the password. Error on the LogOn step will print your credentials to the console when Verbose output is selected. Please confirm Verbose output should be enabled:',
          type: 'confirm',
          askAnswered: true,
          // Ask for this when the verbosity flag is not passed endpoint was not set via a parameter
          when: (responses: any) => {
            return (
              responses.maskPassword === true && responses.verbose === true
            );
          },
          default: false,
        },
    ...
    const clients: Clients = await config(
      responses.baseEndpointUrl || flags.baseEndpointUrl,
    );
    ...
    async function repl() {
      ...
      switch (answer.action) {
        ...
        case 'GetReportParameters':
          ...
          await getReportParametersAction(
            clients,
            logOnResult,
            reportPathOrId,
            flags,
        ...
        case 'ExecuteReport':
          if (retrievedLastReport) {
            ...
            try {
              executeReportResult = await executeReportAction(
                clients,
                logOnResult,
                reportPathOrId,
                flags,
            ...
            if (!executeReportResult?.hasErrors) {
              reportKey =
                executeReportResult?.result[0].ExecuteReportResult.ReportKey;
              retrievedLastReport = false;
            }
          } else {
            console.log(
              chalk.red(
                'Running more than one (1) report is not supported. Please run RetrieveReport first.',
              ),
            );
          }
        ...
        case 'RetrieveReport':
          if (reportKey !== '' && retrievedLastReport === false) {
            ...
            const decodedStream = await decodeStream(
            ...
            console.log(
              chalk.red(
                'You must ExecuteReport before you can RetrieveReport.',
              ),
            );
          break;
